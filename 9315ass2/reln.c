...
PageID addToRelation(Reln r, Tuple t)
{
    ...
    int size=rp->tsigSize;

    Byte *add=giveback(tuplesig);
    Byte *addr=addrInPage(p, pageNitems(p), size);

    memcpy(addr, add, size);
    ...
    Bits thispagesig=newBits(r->params.pm);
    pid=rp->psigNpages-1;
    ...
    add=giveback(thispagesig);
    size=r->params.psigSize;
    addr=addrInPage(p, pageNitems(p)-1, size);
    memcpy(add,addr,size);
    orBits(thispagesig, pagesig);
    add=giveback(thispagesig);
    
    memcpy(addr,add,size);

    putPage(r->psigf, pid, p);
//    if (bitIsSet(thispagesig, 3)){
//        printf("%d\n",addr[0]);
//        showBits(thispagesig);
//        printf("\n");
//    }
    ...
    int i;
    Bits zero=newBits(r->params.bm);
    add=giveback(zero);
    unsetAllBits(thispagesig);
    size=rp->bsigSize;
    if (rp->ntups==1){
        for (i=0;i<rp->pm;i++){
            ...
            rp->nbsigs++;
            addr=addrInPage(p, pageNitems(p), size);
            memcpy(addr,add,size);
            addOneItem(p);
